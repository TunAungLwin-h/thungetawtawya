...
$target_level = (int)($_POST['target_level'] ?? 3);
...
try {
    ...
    // Check if already has Level 3 registration for this year
    ...
    if ($existing) {
        $_SESSION['flash'] = 'You already have a Level 3 registration for this year';
        header('Location: level3.php');
        exit;
    }
    ...
    /* =====================================================
       🔹 ADDED: Generate NEW roll number (LEVEL 3 + YEAR)
       ===================================================== */
    $stmt = $pdo->prepare("
        SELECT roll_number
        FROM registrations
    ...
    // Insert new Level 3 registration
    $stmt = $pdo->prepare("
    ...
    /* =====================================================
       🔹 ADDED: Fetch previous roll correctly
       ===================================================== */
    ...
    header('Location: level3_success.php');
    exit;

} catch (Exception $e) {
    error_log("Reform to Level 3 error: " . $e->getMessage());
    $_SESSION['flash'] = 'Error creating Level 3 registration. Please try again.';
    header('Location: level3.php');
    exit;
}
?>
